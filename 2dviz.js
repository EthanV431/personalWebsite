...
const fsSource = `
    ...
    void main() {
        ...
        cord = matrix * cord;
        float colorG = abs(sin(cord.y * 30.0));
        gl_FragColor = vec4(colorR, colorG, 1.0, 1.0);  
    }
`;
/*
add this back to main above gl_FragColor and replace values with color variables
makes the fancy rainbow thing
float colorR = abs(cos(uTimeFrag  * 2.0));
float pixelCordX = gl_FragCoord.x/screenSize.x;
float pixelCordY = gl_FragCoord.y/screenSize.y;
vec2 cord = vec2(pixelCordX, pixelCordY);
