...
function initScene() {
	...
	loader.load('models/guy.obj', function (object) {

		// attach material
		object.traverse(function (child) {
			if (child.isMesh) {
				child.material = woodMaterial; // Apply the material to each mesh
		...
		group.add(object);
		group1 = group.clone();
		group1.scale.set(2, 2, 2); 
		group1.position.set(4, 3, 2); 
		scene.add(group1);
		render();
	}, onProgress, onError);
	...
	group1 = group.clone();
	group1.scale.set(1, 1, 1); // Set to a more noticeable scale
	group1.position.set(0, 10, 0); // Temporarily set to a more obvious position
	console.log(group1); // Add this line to check the cloned group
	scene.add(group1);
